...
export async function GET(request: NextRequest) {
	...
	try {
		...
		const minGrossPay = searchParams.get('minGrossPay');
		const maxGrossPay = searchParams.get('maxGrossPay');
		const minNetPay = searchParams.get('minNetPay');
		const maxNetPay = searchParams.get('maxNetPay');
		...
		// Add gross pay range filters
		if (minGrossPay || maxGrossPay) {
			whereClause.grossPay = {};
			if (minGrossPay) {
				const minValue = parseFloat(minGrossPay);
				if (!isNaN(minValue)) {
					whereClause.grossPay.gte = minValue;
				}
			}
			if (maxGrossPay) {
				const maxValue = parseFloat(maxGrossPay);
				if (!isNaN(maxValue)) {
					whereClause.grossPay.lte = maxValue;
				}
			}

			// Remove empty object if no valid filters were added
			if (Object.keys(whereClause.grossPay).length === 0) {
				delete whereClause.grossPay;
			}
		}

		// Add net pay range filters
		if (minNetPay || maxNetPay) {
			whereClause.netPay = {};
			if (minNetPay) {
				const minValue = parseFloat(minNetPay);
				if (!isNaN(minValue)) {
					whereClause.netPay.gte = minValue;
				}
			}
			if (maxNetPay) {
				const maxValue = parseFloat(maxNetPay);
				if (!isNaN(maxValue)) {
					whereClause.netPay.lte = maxValue;
				}
			}

			// Remove empty object if no valid filters were added
			if (Object.keys(whereClause.netPay).length === 0) {
				delete whereClause.netPay;
			}
		}
