...
export async function GET(request: NextRequest) {
	...
	try {
		...
		const companyId = searchParams.get('companyId'); // Required parameter for company scoping

		// Validate companyId parameter
		if (!companyId) {
			return withCors(ApiResponse.error('companyId parameter is required', 400), origin);
		}
		...
		const where: any = {
			...
			// Always filter by companyId from param
			companyId: companyId,
		...
		// Super Admin validation: Even SUPER_ADMIN must provide a companyId
		// They cannot access all companies through this endpoint
		if (!companyId) {
			return withCors(ApiResponse.error('companyId is required', 400), origin);
		}

		// Additional validation: If user is not SUPER_ADMIN, ensure they have access to this company
		if (user.role !== 'SUPER_ADMIN') {
			if (!user.companyId) {
				return withCors(ApiResponse.error('No company assigned for this user', 400), origin);
			}
			// Verify that the requested companyId matches the user's company
			if (user.companyId !== companyId) {
				return withCors(ApiResponse.error('Access denied: You do not have permission to access staff records for this company', 403), origin);
			}
		}
		...
		if (includeInactive) {
			// Count active staff for this company
			...
			// Count inactive staff for this company
