/**
 * Deductions Upload API Routes
 * POST /api/engine/deductions/upload/[periodId]?type=TYPE - Upload deductions
 *
...
const VALID_DEDUCTION_TYPES = ['UNION_DUES', 'COOPERATIVE', 'LOAN', 'SALARY_ADVANCE', 'OTHER']
...
/**
 * Parse Excel/CSV file and extract deduction data
 */
async function parseFile(file: File): Promise<DeductionData[]> {
  ...
  const fieldMappings: Record<string, string[]> = {
    ...
    amount: ['amount', 'deductionamount', 'value', 'deduction'],
  ...
  worksheet.eachRow({ includeEmpty: false }, (row, rowNumber) => {
    ...
    const amount = cellToNumber(row.getCell(columnMap.amount).value)

    // Skip rows without staffId or amount
    ...
    const deductionDto: DeductionData = {
      staffId,
      amount,
...
export async function POST(
  ...
  try {
    ...
    const deductionType = searchParams.get('type') ?? undefined

    if (!deductionType || !VALID_DEDUCTION_TYPES.includes(deductionType)) {
      return withCors(
        NextResponse.json(
          {
            success: false,
            message: `Invalid deduction type. Valid types: ${VALID_DEDUCTION_TYPES.join(', ')}`
          },
          { status: 400 }
        ),
        origin
    ...
    if (contentType.includes('multipart/form-data')) {
      ...
      if (data.length === 0) {
        return withCors(
          NextResponse.json(
            { success: false, message: 'No valid data found in the file. Ensure the file has staffId and amount columns.' },
            { status: 400 }
          ),
          origin
        )
    ...
    const result = await deductionsService.bulkImportDeductions(
      periodId,
    ...
    return withCors(
      NextResponse.json({
        success: true,
        message: `${result.created} deduction entries imported successfully`,
        data: {
          ...result,
    ...
    console.error('Upload deductions error:', error?.message || String(error))
    return withCors(
      NextResponse.json(
        { success: false, message: error.message || 'Failed to upload deduction entries' },
        { status: 500 }
      ),
      origin
    )
