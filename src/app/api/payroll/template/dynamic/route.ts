...
export async function GET(request: NextRequest) {
	...
	try {
		...
		return withCors(ApiResponse.success(templates, 'Templates retrieved successfully'), origin);
	} catch (error) {
...
export async function POST(request: NextRequest) {
	...
	try {
		...
		const { companyId, templateName, sections, isSystem = false } = body;
		...
		if (existingTemplate) {
			return withCors(ApiResponse.error(`Template with name "${templateName}" already exists`, 400), origin);
		}
			...
			for (const sectionName of sectionOrder) {
				...
				for (const field of sectionFields) {
					...
					if (!field.systemField) {
						...
						field.systemField = field.displayName.toLowerCase().replace(/[^a-z0-9]+/g, '_');
					}
		...
		return withCors(ApiResponse.success(template, 'Template created successfully'), origin);
	} catch (error) {
...
export async function PUT(request: NextRequest) {
	...
	try {
		...
		const body = await request.json();
		const { templateId, templateName, sections } = body;
		...
		return withCors(ApiResponse.success(template, 'Template updated successfully'), origin);
	} catch (error) {
...
export async function DELETE(request: NextRequest) {
	...
	try {
		...
		if (template._count.payrolls > 0 || template._count.payrollData > 0) {
			return withCors(ApiResponse.error('Cannot delete template that has been used for payroll processing', 400), origin);
		}
