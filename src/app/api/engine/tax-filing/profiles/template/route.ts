...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const instructions = [
      'ΓÇó Staff ID: Must match exactly the Staff ID in the system (required)',
      'ΓÇó State of Residence: Must be a valid Nigerian state name (required) - see States sheet for valid names',
      'ΓÇó JTB TIN: 13-digit Tax Identification Number (optional)',
      'ΓÇó PFA Name: Pension Fund Administrator name (optional)',
      '',
      'ΓÇó Delete the sample rows before uploading your data',
      'ΓÇó Existing profiles will be updated if Staff ID matches',
    ]
