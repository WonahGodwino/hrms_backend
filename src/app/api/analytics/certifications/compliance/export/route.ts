...
// GET /api/analytics/certifications/compliance/export
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    if (format === 'csv') {
      if (!rows.length) {
        const emptyCsv = 'Staff ID,Employee Name,Email,Department,Role,Certification,Type,Authority,Status,Issue Date,Expiry Date,Days to Expiry,Has Document\n'
        return new Response(emptyCsv, {
          headers: {
            'Content-Type': 'text/csv',
            'Content-Disposition': 'attachment; filename="compliance-dashboard-export.csv"',
