...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const quoteCurrency = String(searchParams.get('quoteCurrency') || '').toUpperCase()
    ...
    if (!isValidCurrencyCode(quoteCurrency)) {
      return withCors(ApiResponse.error('Invalid quoteCurrency. Use a valid ISO 4217 code', 400), origin)
    }
    ...
    const baseCurrency = normalizeCurrencyCode(requestedBaseCurrency || company.baseCurrency || 'NGN')
    ...
    return withCors(
      ApiResponse.success({
        ...
        baseCurrencySymbol: getCurrencySymbol(live.baseCurrency),
        quoteCurrency: live.quoteCurrency,
        quoteCurrencySymbol: getCurrencySymbol(live.quoteCurrency),
        pair: `${live.baseCurrency}/${live.quoteCurrency}`,
        rate: live.rate,
        source: live.source,
        asOf: live.asOf
      }, 'Live exchange rate fetched successfully'),
      origin
    )
  } catch (error: any) {
