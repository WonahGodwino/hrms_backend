...
export async function GET(request: NextRequest, { params }: RouteParams) {
  ...
  try {
    ...
    if (!sanitizedId || !validator.isLength(sanitizedId, { min: 1, max: 100 })) {
      return withCors(
        ApiResponse.error('Invalid staff ID', 400),
        origin
      )
    }
    ...
    if (currentUser.role !== 'SUPER_ADMIN') {
      ...
      if (!hasAccess) {
        return withCors(
          ApiResponse.error('You do not have access to this staff record', 403),
          origin
        )
      }
    }
    ...
    const sanitizedResponse = {
      ...
      department: validator.escape(staffRecord.department),
    ...
    return withCors(
      ApiResponse.success(sanitizedResponse, 'Staff record retrieved successfully'),
      origin
    )
  } catch (error) {
...
export async function PATCH(request: NextRequest, { params }: RouteParams) {
  ...
  try {
    ...
    if (currentUser.role !== 'SUPER_ADMIN') {
      ...
      if (!hasAccess) {
        return withCors(
          ApiResponse.error('You do not have access to update this staff record', 403),
          origin
        )
      }
    }
    ...
    if (email !== undefined) {
      if (!email || typeof email !== 'string') {
        return withCors(
          ApiResponse.error('Email must be a non-empty string', 400),
          origin
        )
      }
    ...
    if (firstName !== undefined) {
      if (!firstName || typeof firstName !== 'string') {
        return withCors(
          ApiResponse.error('First name must be a non-empty string', 400),
          origin
        )
      }
      ...
      if (!sanitizedFirstName || !validator.isLength(sanitizedFirstName, { min: 1, max: 100 })) {
        return withCors(
          ApiResponse.error('First name must be 1-100 characters', 400),
          origin
        )
      }
    ...
    if (lastName !== undefined) {
      if (!lastName || typeof lastName !== 'string') {
        return withCors(
          ApiResponse.error('Last name must be a non-empty string', 400),
          origin
        )
      }
      ...
      if (!sanitizedLastName || !validator.isLength(sanitizedLastName, { min: 1, max: 100 })) {
        return withCors(
          ApiResponse.error('Last name must be 1-100 characters', 400),
          origin
        )
      }
    ...
    if (department !== undefined) {
      if (!department || typeof department !== 'string') {
        return withCors(
          ApiResponse.error('Department must be a non-empty string', 400),
          origin
        )
      }
      ...
      if (!sanitizedDepartment || !validator.isLength(sanitizedDepartment, { min: 1, max: 100 })) {
        return withCors(
          ApiResponse.error('Department must be 1-100 characters', 400),
          origin
        )
      }
    ...
    if (position !== undefined) {
      if (!position || typeof position !== 'string') {
        return withCors(
          ApiResponse.error('Position must be a non-empty string', 400),
          origin
        )
      }
      ...
      if (!sanitizedPosition || !validator.isLength(sanitizedPosition, { min: 1, max: 100 })) {
        return withCors(
          ApiResponse.error('Position must be 1-100 characters', 400),
          origin
        )
      }
    ...
    if (phone !== undefined) {
      if (phone !== null && typeof phone !== 'string') {
        return withCors(
          ApiResponse.error('Phone must be a string or null', 400),
          origin
        )
      }
    ...
    if (bankName !== undefined) {
      if (bankName !== null && typeof bankName !== 'string') {
        return withCors(
          ApiResponse.error('Bank name must be a string or null', 400),
          origin
        )
      }
      ...
      if (sanitizedBankName && !validator.isLength(sanitizedBankName, { max: 100 })) {
        return withCors(
          ApiResponse.error('Bank name must be less than 100 characters', 400),
          origin
        )
      }
    ...
    if (accountNumber !== undefined) {
      if (accountNumber !== null && typeof accountNumber !== 'string') {
        return withCors(
          ApiResponse.error('Account number must be a string or null', 400),
          origin
        )
      }
      ...
      if (sanitizedAccountNumber === null && accountNumber !== null) {
        return withCors(
          ApiResponse.error('Invalid account number format', 400),
          origin
        )
      }
    ...
    if (bvn !== undefined) {
      if (bvn !== null && typeof bvn !== 'string') {
        return withCors(
          ApiResponse.error('BVN must be a string or null', 400),
          origin
        )
      }
      ...
      if (sanitizedBVN === null && bvn !== null) {
        return withCors(
          ApiResponse.error('BVN must be 11 digits', 400),
          origin
        )
      }
    ...
    if (isRegistered !== undefined) {
      if (typeof isRegistered !== 'boolean') {
        return withCors(
          ApiResponse.error('isRegistered must be a boolean', 400),
          origin
        )
      }
    ...
    if (role !== undefined) {
      if (!role || typeof role !== 'string') {
        return withCors(
          ApiResponse.error('Role must be a non-empty string', 400),
          origin
        )
      }
      ...
      if (!sanitizedRole || !allowedRoles.includes(sanitizedRole)) {
        return withCors(
          ApiResponse.error(`Role must be one of: ${allowedRoles.join(', ')}`, 400),
          origin
        )
      }
    ...
    if (isActive !== undefined) {
      if (typeof isActive !== 'boolean') {
        return withCors(
          ApiResponse.error('isActive must be a boolean', 400),
          origin
        )
      }
    ...
    if (encodedId !== undefined) {
      if (encodedId !== null && typeof encodedId !== 'string') {
        return withCors(
          ApiResponse.error('Encoded ID must be a string or null', 400),
          origin
        )
      }
      ...
      if (sanitizedEncodedId && !validator.isLength(sanitizedEncodedId, { max: 100 })) {
        return withCors(
          ApiResponse.error('Encoded ID must be less than 100 characters', 400),
          origin
        )
      }
    ...
    const sanitizedResponse = {
      ...
      department: validator.escape(updatedStaff.department),
      position: validator.escape(updatedStaff.position),
    ...
    return withCors(
      ApiResponse.success(sanitizedResponse, 'Staff record updated successfully'),
      origin
    )
  } catch (error) {
    const message = formatError(error)
    ...
    if (message.includes('Unique constraint failed')) {
      return withCors(
        ApiResponse.error('Email or staff ID already exists in this company', 400),
        origin
      )
    }
...
export async function DELETE(request: NextRequest, { params }: RouteParams) {
  ...
  try {
    ...
    if (currentUser.role !== 'SUPER_ADMIN') {
      ...
      if (!hasAccess) {
        return withCors(
          ApiResponse.error('You do not have access to deactivate this staff record', 403),
          origin
        )
      }
    }
    ...
    if (existingStaff.id === currentUser.userId) {
      return withCors(
        ApiResponse.error('You cannot deactivate your own account', 400),
        origin
      )
    }
    ...
    return withCors(
      ApiResponse.success(sanitizedResponse, 'Staff record deactivated successfully'),
      origin
    )
  } catch (error) {
    const message = formatError(error)
