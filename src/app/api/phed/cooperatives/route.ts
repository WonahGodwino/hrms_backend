...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const coops = await (prisma as any).phedCooperative.findMany({
      where: { companyId, isActive: true },
      orderBy: { name: 'asc' },
    })
    return withCors(ApiResponse.success(coops), origin)
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const coop = await (prisma as any).phedCooperative.create({
      data: { companyId, name },
    })
    return withCors(ApiResponse.success(coop, 'Cooperative created', 201), origin)
  } catch (e) { return withCors(handleApiError(e), origin) }
}
