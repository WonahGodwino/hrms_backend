  ...
  async processFile(
    ...
    for (let index = 0; index < data.length; index++) {
      ...
      try {
        ...
        if (!staffRecord) {
          throw new Error(`Staff record not found for ${staffId || rawEmail || rawName}`)
        }
        ...
        const existingPayrollToOverwrite = await prisma.payroll.findFirst({
          where: { staffRecordId: staffRecord.id, month: monthName, year, companyId: companyId },
          orderBy: { createdAt: 'desc' },
        })
        const payrollRecord = existingPayrollToOverwrite
          ? await prisma.payroll.update({ where: { id: existingPayrollToOverwrite.id }, data: { ...payrollData, updatedAt: new Date() } })
