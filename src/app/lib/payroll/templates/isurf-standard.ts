      ...
      try {
        ...
        const payrollRecord = await prisma.payroll.upsert({
          ...
          update: {
            ...
            entertainment,
            utility,
            proratedGrossWithExtra: num(getCell(row, "PRORATED GROSS PAY WITH EXTRA ALL'WCE", canonicalMap)),
            annualPension: pension * 12,
            annualGrossPay: grossPay * 12,
            consolidatedRelief: num(getCell(row, 'Consolidated Relief', canonicalMap)),
            taxableIncome: num(getCell(row, 'TAXABLE INCOME', canonicalMap)),
            ...
            proratedSubTotal: num(getCell(row, 'Prorated Sub Total Invoice', canonicalMap)),
