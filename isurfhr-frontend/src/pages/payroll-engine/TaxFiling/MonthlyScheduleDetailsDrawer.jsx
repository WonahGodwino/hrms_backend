// src/pages/payroll-engine/TaxFiling/MonthlyScheduleDetailsDrawer.jsx
...
import { getStateScheduleDetails, downloadStateSchedule, FILING_STATUS } from '@/services/TaxFilingService';

const MonthlyScheduleDetailsDrawer = ({ open, onClose, periodId, stateCode, companyId }) => {
	...
	const fetchData = useCallback(async () => {
		if (!periodId || !stateCode || !companyId) return;

		setLoading(true);
		setError(null);
		try {
			const response = await getStateScheduleDetails(periodId, stateCode, companyId);
			if (response.data?.success) {
				setScheduleData(response.data.data.scheduleData || null);
	...
	const handleDownload = async (format) => {
		...
		setDownloading(true);
		try {
			const response = await downloadStateSchedule(periodId, stateCode, format, companyId);
			...
			link.href = url;
			link.download = `PAYE-Schedule-${stateCode}-${scheduleData?.period?.year}-${scheduleData?.period?.month}.${format}`;
			...
			setError('Failed to download schedule');
		} finally {
			...
			<Box
				...
				<Box>
					...
					<Typography variant="body2" sx={{ color: isDarkMode ? '#94a3b8' : '#64748b', mt: 0.5 }}>
						{scheduleData?.stateName || stateCode} ({scheduleData?.irsName || 'IRS'}) -{' '}
						{getMonthName(scheduleData?.period?.month)} {scheduleData?.period?.year}
					</Typography>
				</Box>
							...
							<Box
								...
								<Box>
									...
									<Typography variant="h6" sx={{ fontWeight: 600 }}>
										{scheduleData.irsName}
									</Typography>
									<Typography variant="body2" color="text.secondary">
										{scheduleData.stateName}
									</Typography>
								</Box>
							...
							<Box
								...
								<Box>
									...
									<Typography variant="h5" sx={{ fontWeight: 700 }}>
										{scheduleData.totals?.employeeCount || 0}
									</Typography>
								</Box>
								<Box>
									<Typography variant="caption" color="text.secondary">
									...
									<Typography variant="h5" sx={{ fontWeight: 700 }}>
										{formatCurrency(scheduleData.totals?.grossIncome)}
									</Typography>
								</Box>
								<Box>
									<Typography variant="caption" color="text.secondary">
									...
									<Typography variant="h5" sx={{ fontWeight: 700, color: '#22c55e' }}>
										{formatCurrency(scheduleData.totals?.taxAmount)}
									</Typography>
								</Box>
							</Box>
						</Paper>
						...
						<Paper
							...
							<TableContainer sx={{ maxHeight: 400 }}>
								<Table stickyHeader size="small">
									<TableHead>
										<TableRow>
											{['S/N', 'Name', 'TIN', 'Staff ID', 'Gross Income', 'Relief', 'Pension', 'Tax'].map(
												(head) => (
													<TableCell
														key={head}
														align={['Gross Income', 'Relief', 'Pension', 'Tax'].includes(head) ? 'right' : 'left'}
														sx={{
									...
									<TableBody>
										{scheduleData.employees?.map((employee, index) => (
											<TableRow
												...
												<TableCell align="right" sx={{ fontSize: '0.8rem' }}>
													{formatCurrency((employee.consolidatedRelief || 0) + (employee.rentRelief || 0))}
												</TableCell>
												<TableCell align="right" sx={{ fontSize: '0.8rem' }}>
													{formatCurrency(employee.pensionContribution)}
												</TableCell>
												<TableCell
													align="right"
													sx={{ fontSize: '0.8rem', fontWeight: 600, color: '#22c55e' }}>
													{formatCurrency(employee.taxPayable)}
												</TableCell>
											</TableRow>
										))}
									</TableBody>
...
export default MonthlyScheduleDetailsDrawer;
